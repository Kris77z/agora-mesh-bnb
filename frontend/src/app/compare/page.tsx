...
import { compareMarketplaceServices, listMarketplaceServices } from '@/lib/marketplace-api';
...
function CompareContent() {
  ...
  const compareQuery = useSWR(
    selectedIds.length >= 2 ? ['service-comparison', [...selectedIds].sort().join(','), sharedTaskType] : null,
    () => compareMarketplaceServices({ serviceIds: selectedIds, taskType: sharedTaskType }),
  );
